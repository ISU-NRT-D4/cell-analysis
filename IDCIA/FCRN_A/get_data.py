...
def create_hdf5(dataset_name: str,
    ...
    for h5, size in ((train_h5, len(os.listdir('IDCIA/train'))//2), (valid_h5, len(os.listdir('IDCIA/valid'))//2)):
...
def generate_cell_data():
    ...
    image_list = glob(os.path.join('combined', '*cell.*'))
    image_list.sort()

    train_images=glob(os.path.join('IDCIA','train', '*x.png'))
    valid_images=glob(os.path.join('IDCIA','valid', '*x.png'))
    ...
    def fill_h5(h5, images):
        ...
        for i, img_path in enumerate(images):
            ...
            # get label path
            label_path = img_path.replace('.png', 'dots.png')
            # get an image as numpy array
            image = np.array(Image.open(img_path), dtype=np.float32) / 255
            image = np.transpose(image, (2, 0, 1))
    ...
    fill_h5(train_h5, train_images)
    fill_h5(valid_h5, valid_images)
