...
class ImageDataLoader():
    def __init__(self, data_path, gt_path, shuffle=False, gt_downsample=False, pre_load=False):
        ...
        self.root=os.path.join("..","data","IDCIA","images")
        if self.pre_load:
            ...
            for fname in self.data_files:
                ...
                # print(f"Height and width are {ht_1} and {wd_1}")
                img = cv2.resize(img,(int(wd_1),int(ht_1)))
                img = img.reshape((1,1,img.shape[0],img.shape[1]))
                gt_file = h5py.File(os.path.join(self.root,fname).replace('.tiff','.h5').replace('images','ground_truth'))
